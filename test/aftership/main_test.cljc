(ns aftership.main-test
  "Contract + behavioral test for the aftership-compat L4 actor (cljc port).
...
(deftest enum-validation
  (let [s (m/fresh-store)]
    (is (= 400 (second (m/handle-create s "Tracking" {:trackingNumber "t1" :tag "InvalidTag"}))) "Tracking invalid tag")
    (is (= 400 (second (m/handle-create s "Tracking" {:trackingNumber "t1" :deliveryType "invalid"}))) "Tracking invalid deliveryType")
    (is (= 400 (second (m/handle-create s "Checkpoint" {:checkpointTime "2026-06-18T00:00:00Z" :slug "s1" :tag "BadTag"}))) "Checkpoint invalid tag")))

(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "aftership-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'aftership.main-test)]
