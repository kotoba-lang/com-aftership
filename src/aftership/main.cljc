(ns aftership.main
  "Kotodama WASM entrypoint for the Aftership clean-room actor (L4) — Clojure port.
  ...
  facts (`aftership.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "aftership")
(def tier "L4")
...
(def entity-specs
  [{:entity "Tracking" :plural "trackings" :id-prefix "aftershi_tra"
    :fields [:trackingNumber :slug :tag :subtag :deliveryType :active :expectedDelivery :orderId]
    :required [:trackingNumber]
    :coerce {:active :bool}
    :refs {}
    :enums {:tag ["Pending" "InfoReceived" "InTransit" "OutForDelivery" "AttemptFail" "Delivered" "AvailableForPickup" "Exception" "Expired"]
            :deliveryType ["pickup_at_store" "pickup_at_courier" "door_to_door"]}}
   {:entity "Checkpoint" :plural "checkpoints" :id-prefix "aftershi_che"
    :fields [:checkpointTime :slug :tag :subtag :message :location :city :countryIso3]
    :required [:checkpointTime :slug]
    :coerce {}
    :refs {}
    :enums {:tag ["Pending" "InfoReceived" "InTransit" "OutForDelivery" "AttemptFail" "Delivered" "AvailableForPickup" "Exception" "Expired"]}}
   {:entity "Courier" :plural "couriers" :id-prefix "aftershi_cou"
    :fields [:slug :name :phone :webUrl :defaultLanguage]
    :required [:slug :name]
    :coerce {}
    :refs {}}
   {:entity "Notification" :plural "notifications" :id-prefix "aftershi_not"
    :fields [:emails :smses]
    :required [:emails :smses]
    :coerce {}
    :refs {}}
   {:entity "EstimatedDeliveryDate" :plural "estimateddeliverydates" :id-prefix "aftershi_est"
    :fields [:estimatedDeliveryDate :confidenceScore :estimatedDeliveryDateMin :estimatedDeliveryDateMax]
    :required [:estimatedDeliveryDate :confidenceScore]
    :coerce {:confidenceScore :float}
    :refs {}}])
...
(defn as-int-kernel
  ...
  of aftership.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"aftership.<Entity>/<field>\" v ...}. The datomic
...
(defn validate-enum [data field values]
  (when (and (some? (get data field)) (not (contains? (set values) (get data field))))
    {:error {:message (str "invalid " (name field) "; allowed: " (str/join ", " values))
             :type "invalid_request_error"}}))
...
(defn handle-create [store entity data]
  (let [{:keys [fields required coerce id-prefix enums]} (spec-for entity)]
    (or (some-> (reject-unknown data fields) (vector 400))
        (some-> (require-fields data required) (vector 400))
        (or (first (keep (fn [[field values]] (some-> (validate-enum data field values) (vector 400)))
                        enums)))
...
(defn handle-update [store entity id data]
  (let [{:keys [fields enums]} (spec-for entity) rows (query store entity id)]
    (if (empty? rows)
      (not-found)
      (or (some-> (reject-unknown data fields) (vector 400))
          (or (first (keep (fn [[field values]] (some-> (validate-enum data field values) (vector 400)))
...
(defn healthz [] [{:status "ok" :actor "aftership-compat" :tier tier :entities entities} 200])
...
(defn start! [] :aftership-compat/ready)
